...
def unwrap_company_facts(payload: dict) -> dict:
    """SEC companyfacts JSON nests metrics under payload['facts']."""
...
def _periods_for_concept(
    ...
    try:
        units = facts[namespace][concept]["units"]
    except KeyError:
        return []
    for unit_key in ("USD", "usd", "US dollars", "pure"):
        if unit_key in units:
            return units[unit_key]
    return next(iter(units.values()), [])


def extract_metric_annual(
    facts: dict,
    concept: str,
    namespace: str = "us-gaap",
) -> Optional[float]:
    """
    Extract the most recent annual (10-K / FY) value for an XBRL concept.
    Returns value in $M.
    """
    periods = _periods_for_concept(facts, concept, namespace)
    if not periods:
        return None

    annual = [
        p for p in periods
        if "end" in p and p.get("val") is not None
        and (
            p.get("form") in ("10-K", "10-K/A")
            or p.get("fp") == "FY"
        )
    ]
    if not annual:
    ...
    return float(annual[0]["val"]) / 1_000_000


def extract_metric_first(
    facts: dict,
    concepts: list[str],
    namespace: str = "us-gaap",
) -> Optional[float]:
    """Try concepts in order; return the first with a usable annual value."""
    for concept in concepts:
        val = extract_metric_annual(facts, concept, namespace)
        if val is not None:
            return val
    return None


def build_financial_snapshot(facts_payload: dict) -> dict:
    """Build a FinancialSnapshot-compatible dict from EDGAR companyfacts. All values in $M."""
    facts = unwrap_company_facts(facts_payload)

    def get(*concepts: str, namespace: str = "us-gaap") -> float:
        return extract_metric_first(facts, list(concepts), namespace) or 0.0
    ...
    ebitda_proxy = op_income + depreciation if (op_income or depreciation) else 0.0
    op_margin = (op_income / revenue) if revenue else 0.0

    inventory = get("InventoryNet", "InventoryFinishedGoodsNetOfReserves", "Inventory")
    ...
    current_debt = get("DebtCurrent", "ShortTermBorrowings", "CommercialPaper")
    debt = long_debt + current_debt if (long_debt and current_debt) else (long_debt or current_debt)
...
def extract_strategy_with_llm(sections: dict[str, str], ticker: str) -> dict:
    ...
    """
    from mimic.llm import get_openai_client, has_llm_credentials, default_extraction_model

    if not has_llm_credentials() or not (sections.get("item1") or sections.get("item1a")):
        return {
            "stated_strategy": "",
            "risk_factors": [],
            "competitive_moat": "",
