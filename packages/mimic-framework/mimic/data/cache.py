...
# Bump when context shape or SEC parsing changes (invalidates stale caches).
CACHE_SCHEMA = "v2"


def _daily_path(ticker: str) -> Path:
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    return CACHE_DIR / f"{ticker.upper()}_{date.today().isoformat()}_{CACHE_SCHEMA}.json"
...
def _quarterly_path(ticker: str, key: str) -> Path:
    ...
    return CACHE_DIR / f"{ticker.upper()}_{now.year}_Q{quarter}_{key}_{CACHE_SCHEMA}.json"
